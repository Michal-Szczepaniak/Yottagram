...
Item {
    ...
    onWidthChanged: gNextTickTimer.callLater(d.updateAnimationSize)
    onHeightChanged: gNextTickTimer.callLater(d.updateAnimationSize)

    // TODO Would be lovely if we could change those at runtime without recreating the animation
    onLoopsChanged: gNextTickTimer.callLater(d.destroyAndRecreate)
    onClearBeforeRenderingChanged: gNextTickTimer.callLater(d.destroyAndRecreate)
    onFillModeChanged: gNextTickTimer.callLater(d.destroyAndRecreate)
    ...
    onParentChanged: gNextTickTimer.callLater(d.destroyAndRecreate);
